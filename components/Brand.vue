<template>
  <div class="py-4">
    <img src="@/assets/img/curve_top.svg" class="w-full" alt="">

    <div class="bg-primary py-8">
      <div class="subtitle text-white text-center mb-14">
        Produk Ekslusif Kami
      </div>
      <div class="grid grid-cols-2 md:grid-cols-4 gap-8">
        <img src="@/assets/img/bnl_logo.png" alt="" class="mx-auto">
        <img src="@/assets/img/technolas_logo.png" alt="" class="mx-auto">
        <img src="@/assets/img/storz_logo.png" alt="" class="mx-auto">
        <img src="@/assets/img/brumaba_logo.png" alt="" class="mx-auto">
      </div>
    </div>

    <img src="@/assets/img/curve_bottom.svg" class="w-full" alt="">
  </div>
</template>

<script>
export default {
}
</script>

<style></style>
